...
Al crear o actualizar un negocio se puede enviar `category`. Los valores admitidos son `BARBERSHOP`, `HAIRDRESSER`, `BEAUTY`, `WELLNESS`, `HEALTH`, `FITNESS`, `PET_SERVICES`, `EDUCATION`, `PROFESSIONAL_SERVICES` y `OTHERS`. Si se omite durante la creacion se usa `OTHERS`; si se omite durante una actualizacion se conserva la categoria actual. Los negocios anteriores a esta funcionalidad se migran a `OTHERS`.
